...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _conversation_closed(request: Request, exc: ConversationClosed):
        return _problem(status.HTTP_409_CONFLICT, "conversation is no longer active")
    ...
    @app.exception_handler(RecordNotVerified)
    async def _record_not_verified(request: Request, exc: RecordNotVerified):
